...
This script fine-tunes Microsoft's Phi-2 model on your book collection to create 
a writing assistant that matches your preferred style and tone.

Features:
- LoRA/QLoRA fine-tuning for efficient training
- Automatic dataset preparation from .txt files
- Model saving and loading
- Gradio web UI for text generation
- Text continuation, chapter writing, and character suggestions

Usage:
1. Place your .txt book files in the 'books' directory
2. Run this script: python phi2_finetuning.py
3. Use the Gradio UI to generate creative writing
"""
...
def train_model(model, tokenized_dataset, tokenizer, output_dir="./outputs"):
    """Train the model."""
    # Training arguments
...
def save_model_info(model_path: str = "./models/phi2-writer"):
    """Save model information and training details."""
    info = {
        "model_name": "Phi-2 Writing Assistant",
        ...
        "saved_at": str(torch.datetime.now())
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phi-2 Fine-tuning for Creative Writing")
    parser.add_argument("--mode", choices=["train", "inference", "ui"], default="train",
                       help="Mode to run: train, inference, or ui")
    parser.add_argument("--download_books", nargs="+", help="Download books from Project Gutenberg by ID")
    parser.add_argument("--test", action="store_true", help="Run quick test after training")
    
    args = parser.parse_args()
    
    # Download books if specified
    if args.download_books:
        print("Downloading books from Project Gutenberg...")
        download_books_from_gutenberg(args.download_books)
